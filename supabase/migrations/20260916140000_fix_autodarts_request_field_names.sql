-- Live vom Nutzer geliefert: der komplette, unminifizierte Login/Refresh-Code des ECHTEN
-- Autodarts-Web-Clients (src-CUjZg3wP.js, direkt von https://play.autodarts.com/assets/ geladen,
-- oeffentlich und ohne Login abrufbar). Das loest das den ganzen Tag ueber beobachtete Raetsel
-- (jeder Fake-Test-Token bekommt sofort ein sauberes 400 "refresh_token is required", der ECHTE
-- gespeicherte Token haengt dagegen komplett ohne jede Antwort): unser Request-Body verwendet
-- ueberall camelCase `refreshToken`, das tatsaechliche Feld heisst aber `refresh_token`
-- (snake_case) -- der Server sieht unter dem Namen, den er erwartet, nie einen Wert, egal ob Fake
-- oder echt. Gleicher Fehler beim Login: wir schicken `email`, der echte Client schickt
-- `username` (der Wert ist bei Autodarts-Accounts zwar die E-Mail-Adresse, das JSON-Feld selbst
-- heisst aber username):
--   async function l(e,t){return c(`/login`,{username:e,password:t,client_id:s})}
--   async function g(e){return c(`/refresh`,{refresh_token:e,client_id:s})}
-- `client_id:"autodarts-play"` hatten wir in beiden Aufrufen schon korrekt -- nur diese eine
-- Feldnamen-Diskrepanz war neu zu finden.
--
-- Behebt ausschliesslich die Feldnamen im ausgehenden Request-Body in den drei betroffenen
-- Funktionen. Alles andere (Timeouts, Retry-Zaehler, der persistente Debug-Trace aus der letzten
-- Migration) bleibt exakt wie zuletzt.
...
begin
  ...
  from public._autodarts_http_post_polled(
    ...
    jsonb_build_object('refresh_token', v_stored_refresh_token, 'client_id', 'autodarts-play'),
    v_browser_headers,
    25000, 100, 1
  ) h;
  ...
  if p_cloud_email is not null and p_cloud_password is not null then
    begin
      ...
      from public._autodarts_http_post_polled(
        ...
        jsonb_build_object('username', p_cloud_email, 'password', p_cloud_password, 'client_id', 'autodarts-play'),
        v_browser_headers
      ) h;
