...
import { Camera, CameraOff, Target, AlertCircle, Loader2, CheckCircle, XCircle, RotateCcw, ArrowRight, Plus } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useToast } from "@/hooks/use-toast";
import { useAuth } from "@/contexts/AuthContext";
import { useNavigate } from "react-router-dom";
import { playCheckoutSound, playTonPlusSound, play180Sound } from "@/utils/sounds";
...
const CameraPage = () => {
  ...
  const startCamera = useCallback(async () => {
    ...
    } catch { setError("Kamerazugriff verweigert. Bitte erlaube den Zugriff in den Geräteeinstellungen."); }
  }, []);
  ...
  const captureAndAnalyze = useCallback(async () => {
    ...
    try {
      ...
      if (data.error) {
        ...
        if (data.totalScore === 180) play180Sound();
        else if (data.totalScore >= 100) playTonPlusSound();
  ...
  const resetAnalysis = () => { setResult(null); setCapturedImage(null); };
  ...
  const confidenceColor = (c: number) => c >= 0.8 ? "text-secondary" : c >= 0.5 ? "text-accent" : "text-destructive";
  const confidenceLabel = (c: number) => c >= 0.8 ? "Hoch" : c >= 0.5 ? "Mittel" : "Niedrig";

  const totalSessionScore = history.reduce((s, r) => s + r.totalScore, 0);
  const sessionAvg = history.length > 0 ? totalSessionScore / history.length : 0;
  ...
  return (
    <div className="container py-6 animate-slide-up max-w-lg mx-auto">
      ...
      {history.length > 0 && (
        <div className="grid grid-cols-3 gap-2 mb-4">
          <div className="bg-card rounded-lg border border-border p-2 text-center">
            <p className="text-lg font-display text-primary">{history.length}</p>
            <p className="text-[10px] text-muted-foreground">Aufnahmen</p>
          </div>
          <div className="bg-card rounded-lg border border-border p-2 text-center">
            <p className="text-lg font-display">{sessionAvg.toFixed(1)}</p>
            <p className="text-[10px] text-muted-foreground">Ø Score</p>
          </div>
          <div className="bg-card rounded-lg border border-border p-2 text-center">
            <p className="text-lg font-display text-accent">{totalSessionScore}</p>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden mb-4 aspect-video relative">
        ...
        {isStreaming && !capturedImage && (
          <div className="absolute top-3 left-3 flex items-center gap-1.5 bg-primary/80 text-primary-foreground px-2 py-1 rounded-full text-xs font-medium">
            <div className="w-2 h-2 rounded-full bg-primary-foreground animate-pulse" /> LIVE
          </div>
        )}
        {analyzing && (
      ...
      {result && !result.error && result.dartsDetected > 0 && (
        <div className="bg-card border border-primary/30 rounded-xl p-4 mb-4 glow-cyan animate-scale-in">
          <div className="text-center mb-3">
            <p className={`text-5xl font-display ${result.totalScore === 180 ? "text-accent" : "text-primary"}`}>
              {result.totalScore}{result.totalScore === 180 && " 🎯"}
            </p>
            <p className="text-xs text-muted-foreground mt-1">{result.dartsDetected} Dart{result.dartsDetected > 1 ? "s" : ""} erkannt</p>
          </div>

          {/* Confidence */}
          <div className="flex items-center justify-center gap-2 mb-3">
            <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
              <div className={`h-full rounded-full transition-all ${result.overallConfidence >= 0.8 ? "bg-secondary" : result.overallConfidence >= 0.5 ? "bg-accent" : "bg-destructive"}`}
            ...
            {result.darts.map((d, i) => (
              <div key={i} className="flex items-center justify-between bg-muted/50 rounded-lg px-3 py-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`font-display text-lg ${d.multiplier === 3 ? "text-destructive" : d.multiplier === 2 ? "text-secondary" : d.segment === 25 ? "text-accent" : "text-foreground"}`}>
                    {formatDart(d)}
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="font-display text-lg text-primary">{d.points}</span>
                  <span className={`text-xs ${confidenceColor(d.confidence)}`}>{Math.round(d.confidence * 100)}%</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="flex gap-2">
        <Button onClick={isStreaming ? stopCamera : startCamera} className="flex-1 font-display uppercase"
      ...
      {history.length > 0 && (
        <div className="mt-6 bg-card border border-border rounded-xl p-4">
          <h3 className="font-display uppercase text-sm mb-3 text-muted-foreground">📊 Session-Verlauf</h3>
          <div className="space-y-1">
            {history.map((r, i) => (
              <div key={i} className="flex items-center justify-between px-2 py-1.5 rounded bg-muted/30">
                <span className="text-xs text-muted-foreground">#{history.length - i}</span>
                <div className="flex items-center gap-2">
                  {r.darts.map((d, j) => (
                    <span key={j} className={`text-xs font-mono ${d.multiplier === 3 ? "text-destructive" : d.multiplier === 2 ? "text-secondary" : "text-foreground"}`}>
                      {formatDart(d)}
                    </span>
                  ))}
                </div>
                <span className={`font-display text-sm ${r.totalScore === 180 ? "text-accent" : "text-primary"}`}>
                  {r.totalScore}{r.totalScore === 180 && " 🎯"}
                </span>
              </div>
            ))}
          </div>
