...
import { BarChart3, Trophy, Target, TrendingUp, Users, ArrowUpDown, Percent, Flame, Calendar } from "lucide-react";
...
const StatisticsPage = () => {
  ...
  const [sortBy, setSortBy] = useState<"average" | "games_won" | "high_score" | "double_rate">("average");
  ...
  const fetchData = useCallback(async () => {
    const [gamesRes, playersRes] = await Promise.all([
      supabase.from("games").select("*").order("played_at", { ascending: false }).limit(200),
  ...
  const clubStats = useMemo(() => {
    ...
    return { totalGames, totalPlayers, avgOfAverages, bestAvg, bestHighscore, mostGames };
  }, [games, players]);
    ...
    <div className="container py-6 animate-slide-up">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {[
          ...
          { label: "Bester Average", value: `${clubStats.bestAvg.val.toFixed(1)}`, sub: clubStats.bestAvg.name, icon: Flame, color: "text-destructive" },
        ].map(s => (
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        <div className="bg-card rounded-xl p-4 border border-border">
          ...
          <p className="text-xl font-display">{clubStats.mostGames.val}</p>
          <p className="text-xs text-muted-foreground">Meiste Spiele</p>
          <p className="text-[10px] text-primary">{clubStats.mostGames.name}</p>
        </div>
      </div>
      ...
      {players.length >= 2 && (
        <div className="bg-card rounded-xl border border-border p-4 mb-6">
          <h3 className="font-display text-sm uppercase mb-3 text-muted-foreground flex items-center gap-2">
            <ArrowUpDown className="w-4 h-4" /> Spielervergleich
          </h3>
          ...
          {comparisonData && (
            <div>
              <div className="grid grid-cols-3 gap-2 text-center text-xs mb-4">
                <div className="font-semibold text-primary">{comparisonData.p1.emoji} {comparisonData.p1.name}</div>
                <div className="text-muted-foreground">vs</div>
                <div className="font-semibold text-secondary">{comparisonData.p2.emoji} {comparisonData.p2.name}</div>
                {[
                  { label: "Average", v1: Number(comparisonData.p1.average).toFixed(1), v2: Number(comparisonData.p2.average).toFixed(1) },
                  { label: "Highscore", v1: comparisonData.p1.high_score, v2: comparisonData.p2.high_score },
                  { label: "Siege", v1: comparisonData.p1.games_won, v2: comparisonData.p2.games_won },
                  { label: "Doppel %", v1: `${Number(comparisonData.p1.double_rate).toFixed(0)}%`, v2: `${Number(comparisonData.p2.double_rate).toFixed(0)}%` },
                ].map(row => (
                  <>
