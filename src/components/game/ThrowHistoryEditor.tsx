  ...
  return (
    <div className="mt-3 bg-card rounded-xl border border-border p-3">
      <div className="flex items-center justify-between mb-2">
        <p className="text-xs text-muted-foreground uppercase font-display">Würfe · {playerName}</p>
        <button onClick={onToggleEditMode} className="text-xs text-primary flex items-center gap-1">
          <Edit2 className="w-3 h-3" /> {editModeOn ? "Fertig" : "Bearbeiten"}
        </button>
      </div>
                    ...
                    {editModeOn ? (
                      <Popover open={openChipIdx === globalIdx} onOpenChange={(open) => onOpenChipChange(open ? globalIdx : null)}>
                        <PopoverTrigger asChild>
                          <button title="Wert ändern" aria-label="Wert ändern">{chip}</button>
                        </PopoverTrigger>
                        <PopoverContent className="w-64 p-2" align="start">
                          ...
                          <div className="flex gap-1 mb-1.5">
                            <button onClick={() => onEditThrow(globalIdx, 0, 1)} className="flex-1 py-1.5 rounded-md text-xs font-bold bg-accent/15 text-accent hover:bg-accent/25">Miss</button>
                            <button onClick={() => onEditThrow(globalIdx, 25, 1)} className="flex-1 py-1.5 rounded-md text-xs font-bold bg-accent/15 text-accent hover:bg-accent/25">Bull</button>
                            <button onClick={() => onEditThrow(globalIdx, 25, 2)} className="flex-1 py-1.5 rounded-md text-xs font-bold bg-accent/15 text-accent hover:bg-accent/25">Bullseye</button>
                          </div>
                          <Button variant="destructive" size="sm" onClick={() => onDeleteThrow(globalIdx)} className="w-full gap-1.5">
                            <X className="w-3.5 h-3.5" /> Wurf löschen
                          </Button>
                        </PopoverContent>
                      </Popover>
                    ) : chip}
